...
% a text file with label coordinate information on DNA objects. The
% algorithm is extended with DNA object lumping (fragmentation) and label 
% separation (if two label objects touch each other) capability. 
%
...
function label_coordinates_lump(x1, y1, x2, y2, tolerance)
...
% Maximum number of pixels allowed for 'DNA_object' lumping (predefined).
dna_gap = 10;
...
for i = 1:length(dna_length_p)
    ...
    DNA_objects(i).lumped_number = 0;

end

% Iterate through all rows (DNA objects) and create 'DNA_object' data 
% structures with pre-defined fields using lumping.
disp('--> Creating DNA object structure');
fprintf('\n');

% Open a textfile to record lumping information.
fileID = fopen('lumping_stats.txt','w');
fprintf(fileID, '%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\r\n', 'ap', 'xp', 'yp', 'op', 'as', 'xs', 'ys', 'os', 'gap', 'an', 'xn', 'yn');

for m = 1:length(dna_length_p)
    
    % Check if the 'DNA_object' has been lumped or not; if so - SKIP.
    if DNA_objects(m).lumped_number == 0;    

        % Initialize label counter.
        counter = 0;
    
        for n = m+1:length(dna_length_p)

            % Check if the 'DNA_object' has been lumped or not; if so - SKIP.
            if DNA_objects(n).lumped_number == 0;  
            
                % Get primary 'DNA_object' major axis length ('ap') and centroid
                % coordinates ('xp', 'yp').
                ap = DNA_objects(m).major_axis_length;
                xp = DNA_objects(m).x_centroid;
                yp = DNA_objects(m).y_centroid;
                op = DNA_objects(m).object_number;

                % Calculate theoretical y-intercept of line defined by DNA segment.
                b = yp - (tand(alpha) * xp);

                % Get secondary 'DNA_object' major axis length ('as') and centroid
                % coordinates ('xs', 'ys').
                as = DNA_objects(n).major_axis_length;
                xs = DNA_objects(n).x_centroid;
                ys = DNA_objects(n).y_centroid;
                os = DNA_objects(n).object_number;

                % Evaluate if DNA fragments are on the same line; use 5% threshold here (variable).
                b_tilda = ys - (tand(alpha) * xs);          

                if abs((b - b_tilda) / b) < (tolerance / 100)  

                    % If intersecting circles give two points (in the vicinity).
                    I = circcirc(xp, yp, (dna_gap + ap) / 2, xs, ys, (dna_gap + as) / 2);
                    if isnan(I(1)) == 0 

                       % Precisely calculate major axis of lumped 'DNA_object' 
                       % using the distance (end; start) points of the cojoint DNA
                       % fragments by calculating the actual gap size (in pixels).
                       [XP, YP] = linecirc(tand(alpha), b, xp, yp, ap / 2);
                       [XS, YS] = linecirc(tand(alpha), b_tilda, xs, ys, as / 2);

                       d1 = pdist([XP(1), YP(1); XS(1), YS(1)], 'euclidean');
                       d2 = pdist([XP(1), YP(1); XS(2), YS(2)], 'euclidean');
                       d3 = pdist([XP(2), YP(2); XS(1), YS(1)], 'euclidean');
                       d4 = pdist([XP(2), YP(2); XS(2), YS(2)], 'euclidean');

                       gap = min([d1 d2 d3 d4]);

                       % Calculate centroid of lumped 'DNA_object' using the midpoint rule. 
                       [M, I] = max([d1 d2 d3 d4]);

                       if I == 1
                           XP = XP(1); YP = YP(1); XS = XS(1); YS = YS(1);                       
                       elseif I == 2
                           XP = XP(1); YP = YP(1); XS = XS(2); YS = YS(2);
                       elseif I == 3 
                           XP = XP(2); YP = YP(2); XS = XS(1); YS = YS(1); 
                       else
                           XP = XP(2); YP = YP(2); XS = XS(2); YS = YS(2); 
                       end

                       new_centroid = bsxfun(@plus, [XP YP], [XS YS]) / 2;

                       % Update primary 'DNA_object' with lumped parameters.
                       DNA_objects(m).major_axis_length = gap + ap + as;
                       DNA_objects(m).x_centroid = new_centroid(1);
                       DNA_objects(m).y_centroid = new_centroid(2);

                       % Note that kth 'DNA_object' has been lumped, i.e., do not 
                       % use it again when iterating through all DNA onjects. 
                       DNA_objects(n).lumped_number = 1;
                       counter = counter + 1;

                       % PRINT TO FILE.
                       fprintf(fileID,'%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f%6.2f\t%6.2f\t%6.2f\t%6.2f\r\n',...
                                      [ap; xp; yp; op; as; xs; ys; os; gap; gap + ap + as; new_centroid(1); new_centroid(2)]); 
...
% Determine the average label length (AL).
AL = mean(label_length_p);
...
for j = 1:length(label_length_p)    
    ...
    % If any label larger than twice the AL, split it into two distinct
    % labels and recalculate their respective centroid coordinates.         
    if label_length_p(j) > 2 * AL 
          
        % First split label.
        label_objects(loc).major_axis_length = label_length_p(j) / 2;
        label_objects(loc).x_centroid = label_x_centroid(j) - label_length_p(j) / 4;    
        label_objects(loc).y_centroid = label_y_centroid(j); 
        label_objects(loc).object_number = label_x_object_id(j); 
        
        % Update label object counter.
        loc = loc + 1;
    ...
    % Condition when label object is a singular entity.    
    else
        
        label_objects(loc).major_axis_length = label_length_p(j);
        label_objects(loc).x_centroid = label_x_centroid(j);    
        label_objects(loc).y_centroid = label_y_centroid(j); 
        label_objects(loc).object_number = label_x_object_id(j); 
...
fileID = fopen('coords_pixel_lumped.txt','w');
fprintf(fileID, '%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\t%6s\r\n', 'al', 'xc', 'yc', 'do', 'ao', 'xl', 'yl', 'lo', 'counter');

% For each 'DNA_object', check the number of labels residing on them.
for k = 1:length(DNA_objects)
    
    % Check if the 'DNA_object' has been lumped or not; if so - SKIP.
    if DNA_objects(k).lumped_number == 0;  
...
fileID = fopen('coords_micron_lumped.txt','w');
...
for m = 1:length(DNA_objects)
    ...
    if isempty(DNA_objects(m).label_set) ~= 1
        if length(DNA_objects(m).label_set) > 1
            ...
            % Iterate through labels specific for a single DNA fragment.
